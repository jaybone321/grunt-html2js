...
module.exports = function (grunt) {
	...
	var compileTemplate = function (moduleName, filepath, options) {
		...
		var compiled = '';
		var cacheProvider = options.customProvider && options.customProvider.length > 0 ? options.customProvider : defaultCache;
		...
		compiled += indentString + cacheProvider + '.put(' + quoteChar + moduleName + quoteChar +
	...
	grunt.registerMultiTask('html2js', 'Compiles Angular-JS templates to JavaScript.', function () {
		...
		this.files.forEach(function (f) {
			...
			if (options.singleModule) {
				if (options.target === 'js') {
					bundle = "angular.module('" + targetModule + "', []).run(['" + cacheProvider + "', function(" + cacheProvider + ") {\n";
					modules += '\n}]);\n';
				} else if (options.target === 'coffee') {
					bundle = "angular.module('" + targetModule + "', []).run(['" + cacheProvider + "', (" + cacheProvider + ") ->\n";
					modules += '\n])\n';
				}
